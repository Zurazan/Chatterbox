...
public static class Util
{
	...
	public static bool IsWindowFocused
	{
		get
		{
			nint activatedHandle = GetForegroundWindow();
	...
	public static string DecompressString(string s, string prefix = "")
	{
		...
		using MemoryStream ms = new MemoryStream(Convert.FromBase64String(s.Substring(prefix.Length)));
		using GZipStream gs = new GZipStream(ms, CompressionMode.Decompress);
		using StreamReader r = new StreamReader(gs);
		return r.ReadToEnd();
	}
	...
	public static object Cast(this Type type, object data)
	{
		...
		return Expression.Lambda(Expression.Block(Expression.Convert(Expression.Convert(dataParam, data.GetType()), type)), dataParam).Compile().DynamicInvoke(data);
	}
