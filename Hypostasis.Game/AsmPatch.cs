...
public sealed class AsmPatch : IDisposable
{
	...
	public AsmPatch(nint address, IReadOnlyCollection<byte?> bytes, bool startEnabled = false)
	{
		if (address != IntPtr.Zero)
		{
			...
			int skip = bytes.Count - trimmedBytes.Length;
			address += skip;
			Address = address;
			byte[] oldBytes = default(byte[]);
			SafeMemory.ReadBytes((IntPtr)address, trimmedBytes.Length, out oldBytes);
			OldBytes = oldBytes;
	...
	private static byte?[] ParseByteString(string bytesString)
	{
		...
		{
			string s = bytesString.Substring(i, 2);
			byte?[] array = bytes;
			int num = i / 2;
			bool flag = ((s == "??" || s == "**") ? true : false);
			array[num] = ((!flag) ? new byte?(byte.Parse(s, NumberStyles.AllowHexSpecifier)) : ((byte?)null));
		}
		return bytes;
	}
