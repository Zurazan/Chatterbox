...
public class Counter : CounterBase
{
	...
	public string GetTitleFreqText()
	{
		...
		for (int i = 1; i <= TitleFreqThreshold; i++)
		{
			...
			if (freq != lastFreq)
			{
				...
				text += $"Count >= {i}, display title per {((freq == 0) ? "trigger" : $"{freq} triggers")}.\n";
				lastFreq = freq;
			}
		}
		int curFreq = GetTitleFreq(Amount);
		return text + $"\nCurrent Count = {Amount}, display title per {((curFreq == 0) ? "trigger" : $"{curFreq} triggers")}.";
	}
	...
	public string GetToastFreqText()
	{
		...
		for (int i = 1; i <= ToastFreqThreshold; i++)
		{
			...
			if (freq != lastFreq)
			{
				...
				text += $"Count >= {i}, display toast per {((freq == 0) ? "trigger" : $"{freq} triggers")}.\n";
				lastFreq = freq;
			}
		}
		int curFreq = GetToastFreq(Amount);
		return text + $"\nCurrent Count = {Amount}, display toast per {((curFreq == 0) ? "trigger" : $"{curFreq} triggers")}.";
	}
	...
	public string GetEchoFreqText()
	{
		...
		for (int i = 1; i <= EchoFreqThreshold; i++)
		{
			...
			if (freq != lastFreq)
			{
				...
				text += $"Count >= {i}, output echo message per {((freq == 0) ? "trigger" : $"{freq} triggers")}.\n";
				lastFreq = freq;
			}
		}
		int curFreq = GetEchoFreq(Amount);
		return text + $"\nCurrent Count = {Amount}, output echo message per {((curFreq == 0) ? "trigger" : $"{curFreq} triggers")}.";
	}
	...
	private int GetCurrentFrequency(int minFreq, int maxFreq, int freqThreshold, int count)
	{
		...
		float raw = Lerp(minFreq, maxFreq, t);
		return Math.Max(1, (int)(Math.Round((double)(int)MathF.Round(raw) / 5.0) * 5.0));
	}
