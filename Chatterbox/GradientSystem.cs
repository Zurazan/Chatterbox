...
public static class GradientSystem
{
	...
	static GradientSystem()
	{
		...
		{
			(string, string) s = ColourSets[i];
			ColourLists.Add(new GradientStyle(s.Item1 + " (Wave)", s.Item2, GradientAnimationStyle.Wave)
			{
				ColourSet = i
			});
			ColourLists.Add(new GradientStyle(s.Item1 + " (Pulse)", s.Item2, GradientAnimationStyle.Pulse)
			{
				ColourSet = i
			});
		}
	...
	public static RGB GetColourRGB(GradientStyle style, int chrIndex, int throttle, bool animate = true)
	{
		...
		byte[,] rainbowColors = style.Colours;
		long num = (animate ? (TimeSinceStart.ElapsedMilliseconds / 15) : 0);
		int m = ((style.AnimationStyle != GradientAnimationStyle.Pulse) ? 1 : 0);
		long i = (num / throttle + chrIndex * m) % rainbowColors.GetLength(0);
		return new RGB(rainbowColors[(int)checked((nint)i), 0], rainbowColors[(int)checked((nint)i), 1], rainbowColors[(int)checked((nint)i), 2]);
	}

	public static Vector3 GetColourVec3(int rainbowMode, int offset, int throttle)
	{
		RGB rgb = GetColourRGB(rainbowMode, offset, throttle);
		return new Vector3((float)(int)rgb.R / 255f, (float)(int)rgb.G / 255f, (float)(int)rgb.B / 255f);
	}

	public static Vector3 GetColourVec3(GradientStyle style, int offset, int throttle)
	{
		RGB rgb = GetColourRGB(style, offset, throttle);
		return new Vector3((float)(int)rgb.R / 255f, (float)(int)rgb.G / 255f, (float)(int)rgb.B / 255f);
	}
	...
	public static GradientStyle? GetStyle(int gradientColourSet, GradientAnimationStyle? gradientAnimationStyle)
	{
		...
		if (!GradientStyles.TryGetValue((gradientColourSet, gradientAnimationStyle), out GradientStyle style))
		{
			(string, string) colourSet = ColourSets[gradientColourSet];
			DefaultInterpolatedStringHandler defaultInterpolatedStringHandler = new DefaultInterpolatedStringHandler(3, 2);
			defaultInterpolatedStringHandler.AppendFormatted(colourSet.Item1);
			...
			defaultInterpolatedStringHandler.AppendLiteral(")");
			style = new GradientStyle(defaultInterpolatedStringHandler.ToStringAndClear(), colourSet.Item2, gradientAnimationStyle ?? GradientAnimationStyle.Static)
			{
