...
using ClientActionManager = FFXIVClientStructs.FFXIV.Client.Game.ActionManager;
...
public struct ActionManager : IHypostasisStructure
{
	...
	public unsafe static bool CanUseActionOnGameObject(uint actionID, GameObject* o)
	{
		...
		{
			LuminaAction? rowOrDefault = DalamudApi.DataManager.GetExcelSheet<LuminaAction>((ClientLanguage?)null, (string)null).GetRowOrDefault(actionID);
			if (rowOrDefault.HasValue)
			{
				LuminaAction valueOrDefault = rowOrDefault.GetValueOrDefault();
				return valueOrDefault.TargetArea;
			}
			return false;
		}
		return true;
