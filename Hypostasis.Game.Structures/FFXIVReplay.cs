...
public struct FFXIVReplay
{
	...
	public struct Header
	{
		...
		public ContentFinderCondition ContentFinderCondition
		{
			get
			{
				return DalamudApi.DataManager.GetExcelSheet<ContentFinderCondition>((ClientLanguage?)null, (string)null).GetRow((uint)contentID);
			}
		}

		public unsafe ClassJob LocalPlayerClassJob
		{
			get
			{
				return DalamudApi.DataManager.GetExcelSheet<ClassJob>((ClientLanguage?)null, (string)null).GetRow((uint)jobs[playerIndex]);
			}
		}

		public IEnumerable<ClassJob> ClassJobs => Enumerable.Range(0, 8).Select(GetJobSafe).TakeWhile((byte id) => id != 0)
			.Select(delegate(byte id)
			{
				return DalamudApi.DataManager.GetExcelSheet<ClassJob>((ClientLanguage?)null, (string)null).GetRow((uint)id);
			});
